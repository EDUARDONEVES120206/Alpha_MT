...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['excluir'])) {
        $sql = "DELETE FROM tbtrapezio WHERE login_usuario = '$login_usuario'";
        
        if ($bancoDeDados->executaSQL($sql)) {
            echo "<script language='javascript' type='text/javascript'>alert('Ficha de treino para trapezio excluída com sucesso.');window.location.href='ficha_trapezio';</script>";
        } else {
    ...
    <h1>Excluir Ficha de Treino para Trapezio</h1>
    
    <form action="excluir_ficha_trapezio.php" method="POST">
        <p>Você tem certeza de que deseja excluir a ficha de treino para trapezio?</p>
        <input type="submit" name="excluir" value="Sim, Excluir">
        <a href="ficha_trapezio.php">Não, Voltar</a>
    </form>
