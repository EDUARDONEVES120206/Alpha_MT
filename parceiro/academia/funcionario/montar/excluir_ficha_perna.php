...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['excluir'])) {
        $sql = "DELETE FROM tbperna WHERE login_usuario = '$login_usuario'";
        
        if ($bancoDeDados->executaSQL($sql)) {
            echo "<script language='javascript' type='text/javascript'>alert('Ficha de treino para pernas excluída com sucesso.');window.location.href='ficha_perna';</script>";
        } else {
    ...
    <h1>Excluir Ficha de Treino para Pernas</h1>
    
    <form action="excluir_ficha_perna.php" method="POST">
        <p>Você tem certeza de que deseja excluir a ficha de treino para pernas?</p>
        <input type="submit" name="excluir" value="Sim, Excluir">
        <a href="ficha_perna.php">Não, Voltar</a>
    </form>
