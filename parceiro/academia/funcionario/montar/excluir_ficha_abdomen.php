...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['excluir'])) {
        $sql = "DELETE FROM tbabdomen WHERE login_usuario = '$login_usuario'";
        
        if ($bancoDeDados->executaSQL($sql)) {
            echo "<script language='javascript' type='text/javascript'>alert('Ficha de treino para abdômen excluída com sucesso.');window.location.href='ficha_biceps';</script>";
        } else {
    ...
    <h1>Excluir Ficha de Treino para Abdômen</h1>
    
    <form action="excluir_ficha_abdomen.php" method="POST">
        <p>Você tem certeza de que deseja excluir a ficha de treino para abdômen?</p>
        <input type="submit" name="excluir" value="Sim, Excluir">
        <a href="ficha_abdomen.php">Não, Voltar</a>
    </form>
