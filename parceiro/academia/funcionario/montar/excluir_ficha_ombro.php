...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['excluir'])) {
        $sql = "DELETE FROM tbombro WHERE login_usuario = '$login_usuario'";
        
        if ($bancoDeDados->executaSQL($sql)) {
            echo "<script language='javascript' type='text/javascript'>alert('Ficha de treino para ombro excluída com sucesso.');window.location.href='ficha_ombro';</script>";
        } else {
    ...
    <h1>Excluir Ficha de Treino para Ombro</h1>
    
    <form action="excluir_ficha_ombro.php" method="POST">
        <p>Você tem certeza de que deseja excluir a ficha de treino para ombro?</p>
        <input type="submit" name="excluir" value="Sim, Excluir">
        <a href="ficha_ombro.php">Não, Voltar</a>
    </form>
